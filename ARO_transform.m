...
function [w]=ARO_transform(f,xk,dt,lambda,T, epsilon, maxIter)
f = double(f);
...
Tyk = imfilter(exp(xk),T,'symmetric','same'); 
f_Tyk_2 = sum(sum((f./Tyk).^2)); 
w = log(f_Tyk_2/ sum(sum(f./Tyk)))*ones(Image_w,Image_h); 
...
% EPSILON REGUALRIZATION,
        ...
        Tewxk = imfilter(exp(w(2:end-1,2:end-1)+xk),T,'symmetric','same'); 
        fidelity = -f./(Tewxk).^2.*(f./Tewxk-1);
        fidelity = imfilter(fidelity,T,'symmetric','same').*exp(w(2:end-1,2:end-1)+xk);
%update interior grid points            
        w(2:end-1,2:end-1)=1./(1+dt.*(c1+c2+c3+c4)).*...
            (w(2:end-1,2:end-1) - dt.*lambda.*fidelity+...
