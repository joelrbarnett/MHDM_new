...
title(['Degraded Image. RMSE=',num2str(noisyRMSE),', SNR=',num2str(noisySNR)],'FontSize',font)
if saveFlag==1
...
plot(0:numScales-1,rmse_final,'b-o','LineWidth',2)
...
yyaxis right
ax=gca; ax.FontSize=16;
plot(0:numScales-1, snr,'r--d','LineWidth',2)
ylabel('SNR','FontSize',22)
legend({'RMSE','SNR'},'FontSize',18)
subplot(1,2,2)
semilogy(0:numScales-1, stopCrit,'k-d','LineWidth',2)
ax=gca; ax.FontSize=16;
hold on
...
xlabel('Multiscales: k','FontSize',22)
if tightFlag(1)==1
    title(['(D(F_{data},Tx_k)+\alpha_kTV(log(x_k)))/D(F_{data},Tu), k^*=',num2str(k_star-1)],'FontSize',22)    
else
    title(['D(F_{data},Tx_k)/D(F_{data},Tu), k^*=',num2str(k_star-1)],'FontSize',22)
end
