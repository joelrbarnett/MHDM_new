...
for j=[7] %loop over all images
    close all;
    %filenames for saving
    filePrefix="AA_orig/"+fileNames(j)+"_high_noise/";
...
%     F_data=double(F_data);
    ...
    numScales=1;
    %algo parameters
    maxIters=10000; %time iterations in solving for wk
    dt=0.0025; %0.025; %timestep
    epsilon= 0.01; %for regularizing TV
    lambda0=1; %intial lambda
    alp0=1;
    ...
    a=1; %gamma noise with mean 1, standard deviation 0.2. 
    ...
    for k=1:numScales    
        ...
        [uk,energy]=AA_orig(F_data,xk,dt,lambda,T,epsilon,maxIters);
