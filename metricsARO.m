...
function [xk_f_norm2,rmse_final,stopCrit,snr]= metricsARO(F_orig,F_data,xkArray,T,numScales,tightFlag)
[m,n]=size(F_orig);
delta_2= norm(F_data./imfilter(F_orig,T,'symmetric','same') -ones(size(F_orig)),'fro')^2;
...
for k=1:numScales
    ...
    H_f_data_Txk=norm(F_data./imfilter(xkArray(:,:,k),T,'symmetric','same')-ones(size(F_orig)),'fro')^2;
    if tightFlag(1)==0 %stopping criteria for regular 
        stopCrit(k)= (H_f_data_Txk)/(delta_2);%ratio of bregman distances
    elseif tightFlag(1)==1 %stopping criteria for tight and refined versions
        alp = tightFlag(2)/(k)^(3/2);
        stopCrit(k)= (H_f_data_Txk+ alp*TV(log(xkArray(:,:,k))))/(delta_2);%ratio of bregman distances
    end
