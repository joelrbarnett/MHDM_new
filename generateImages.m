...
for f=fileNames
    ...
    vars=dir(['./SO_constInit/', file,'_noise_additive/', '*.mat']); %get all .mat files
    load([vars.folder,'/',vars.name]); %load

    %For generating table with minRMSE and SNR values for kmin and kstar
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     plotFigsOsher(F_orig, F_data, xkArray,params,filePrefix,figPrefix,saveFlag,tightFlag); 
    ...
    plotZoomImages(F_orig, F_data, xkArray,zoomBox,filePrefix,figPrefix,saveFlag,tightFlag)
    
    close all; 
    clear F_orig F_data xkArray params filePrefix figPrefix saveFlag tightFlag;
    
    for scheme = ["tight","refined"]
        %load osher shi recoveries
        vars=dir(['./SO_constInit/', file,'_noise_',char(scheme),'/', '*.mat']); %get all .mat files
        load([vars.folder,'/',vars.name]); %load
        %For generating table with minRMSE and SNR values for kmin and kstar
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [xk_f_norm2,rmse_final,stopCrit,snr]= metrics(F_orig,F_data,squeeze(xkArray),numScales,tightFlag);
      ...
      %%%%%%%%%%%%%%%%%%%%%%% end table generating code
        ...
        %for plotting/saving figures
        %plotFigsOsher(F_orig, F_data, xkArray,params,filePrefix,figPrefix,saveFlag,tightFlag);
        
        %for plotting zoomed/detail figs        
        zoomBox=zoomBoxes(idx,:);
        plotZoomImages(F_orig, F_data, xkArray,zoomBox,filePrefix,figPrefix,saveFlag,tightFlag)
...
for f=fileNames
    file = char(f);
    %load AA recoveries
    vars=dir(['./AA/', file,'_noise/', '*.mat']); %get all .mat files
    load([vars.folder,'/',vars.name]); %load
...
%     %For plotting/saving figs
    ...
    for scheme = ["refined","tight"]
        %load AA recoveries
        if scheme == "refined"
            vars=dir(['./AA_varInit/', file,'_noise_',char(scheme),'/', '*.mat']); %get all .mat files
        else
            vars=dir(['./AA/', file,'_noise_',char(scheme),'/', '*.mat']); %get all .mat files
        end
        load([vars.folder,'/',vars.name]); %load
        ...
        elseif scheme=="refined"
            ...
            snrKstarResults{7,idx}=snr(k_star);
            rmseResults{7,idx}=rmse_final(mink);
            rmseKstarResults{7,idx}=rmse_final(k_star);
        end
...
for f=fileNames
    file = char(f);
    %load recoveries
    vars=dir(['./ARO_optInit/', file,'_noise_additive/', '*.mat']); %get all .mat files
    load([vars.folder,'/',vars.name]); %load
    snrResults{1,idx}=file;
    snrKstarResults{1,idx}=file;
    rmseResults{1,idx}=file;
    ...
    snrKstarResults{8,idx}=snr(k_star);
    rmseResults{8,idx}=rmse_final(mink);
    rmseKstarResults{8,idx}=rmse_final(k_star);
    ...
    for scheme = ["tight"]
        %load ARO recoveries
        vars=dir(['./ARO_optInit/', file,'_noise_',char(scheme),'/', '*.mat']); %get all .mat files
        load([vars.folder,'/',vars.name]); %load
        ...
        if scheme=="tight"
            snrResults{9,idx}=snr(mink);
            snrKstarResults{9,idx}=snr(k_star);
            rmseResults{9,idx}=rmse_final(mink);
            rmseKstarResults{9,idx}=rmse_final(k_star);
        end
        %For plotting and saving images
        %plotFigsARO(F_orig, F_data, xkArray,params,T,filePrefix,figPrefix,saveFlag,tightFlag) 
...
for f=fileNames
    file = char(f);
    %load recoveries
    vars=dir(['./TNV_varInit/', file,'/', '*.mat']); %get all .mat files
    load([vars.folder,'/',vars.name]); %load
    snrResults{1,idx}=file;
    snrKstarResults{1,idx}=file;
    rmseResults{1,idx}=file;
    ...
    snrResults{10,idx}=snr(mink);
    %snrKstarResults{10,idx}=snr(k_star);
    rmseResults{10,idx}=rmse_final(mink);
...
%     for scheme = ["tight"]
%         %load ARO recoveries
%         vars=dir(['./TNV/', file,'_',char(scheme),'/', '*.mat']); %get all .mat files
%         load([vars.folder,'/',vars.name]); %load
%         
%         %for storing results in matrix for generating table or resulsts
%         %%%%%%%%%%%%%%%%%%%%%%%%%
...
%         end
%         if scheme=="tight"
%             snrResults{11,idx}=snr(mink);
%             %snrKstarResults{11,idx}=snr(k_star);
%             rmseResults{11,idx}=rmse_final(mink);
...
% writetable(tab1,'snrResults_v2.csv')
% writetable(tab2,'snrKstarResults_v2.csv')
% writetable(tab3,'rmseResults_v2.csv')
% writetable(tab4,'rmseKstarResults_v2.csv')
