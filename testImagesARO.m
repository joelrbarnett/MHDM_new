...
% model: minimize int[lambda*((f/u-1)^2] + TV(u) over u. Recover
% u=exp(w). 
...
for j=ims %1:length(images) %loop over all images
    ...
    filePrefix="ARO_blur_optInit/"+fileNames(j)+"_noise_additive/";
...
%     F_data=double(F_data);
    ...
    for k=1:numScales    
        ...
        uk=ARO(F_data,xk,dt,lambda,T,epsilon, maxIters);
